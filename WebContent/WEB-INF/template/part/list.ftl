		...
		<script type="text/javascript">
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					...
					datatype: "json",
					height: 250,
					colNames:[' ', 'sn','配件名称', '配件规格', '配件种类', '配件进货价', '配件批发价', '配件销售价', '供应商'],
					colModel:[
						...
						{name:'standard',index:'standard', width:60, align:"center", editable: true},
						{name:'partCategory',index:'partCategory', width:60, align:"center", editable: false, formatter:function(cellvalue, options, rowObject) { if (cellvalue == null) return "-"; else return ""+cellvalue.name;}},
						{name:'purchasePrice',index:'purchasePrice', width:60, align:"center", editable: true},
						{name:'wholesalePrice',index:'wholesalePrice', width:60, align:"center", editable: true},
						{name:'salePrice',index:'salePrice', width:60, align:"center", editable: true},
						{name:'supplier',index:'supplier', width:60, align:"center", editable: false, formatter:function(cellvalue, options, rowObject){ if (cellvalue == null) return "-"; else return ""+cellvalue.name;}}
					], 
					jsonReader: {
					...
					ondblClickRow: function(id){  
						window.open("${basePath}/admin/part/update?id=" + id);
					},
					...
					},
					{
						...
						beforeShowForm: function(e){
							...
							if (r) {
								...
								window.open("${basePath}/admin/part/show?id=" + id);
